...
UA = "linwuyen-alpha-engine/4.6 (+https://github.com/linwuyen/stock)"
...
def resilient_fetch(url):
    record = telemetry.setdefault(url, {"primary_attempts": 0, "fallback_used": None, "errors": [], "optional_degraded": False})
    try:
        raw, attempts, errors = request_bytes(url, "application/json")
        record["primary_attempts"] = attempts
        record["errors"].extend(errors)
        rows = parse_json_rows(raw)
    ...
    if fallback:
        try:
            raw, attempts, errors = request_bytes(fallback, "text/csv,*/*")
...
def annotate_screen():
    ...
    for market, endpoints in scan_market.SOURCES.items():
        for label, url in endpoints.items():
            ...
            if rec.get("optional_degraded"):
                ...
                health["error"] = "Official income endpoint unavailable after retries/fallback; discovery continued with explicit earnings verification requirement."
            else:
    ...
    screen.setdefault("rules", {})["source_failure_policy"] = (
        "profiles/quotes/valuation/revenue fail closed after retries and official fallback; "
...
def main():
    ...
    scan_market.main()
    annotate_screen()
    print("resilient transport telemetry persisted")
