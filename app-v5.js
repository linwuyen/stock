...
function renderHero(data){
  ...
  $('#decisionMeta').innerHTML=[badge(`Alpha ${s.score??'—'}`),badge(`Confidence ${s.confidence_score??'—'}`),badge(`ER ${pct(v.expected_return_pct)}`),badge(`Spread ${pct(s.alpha_spread_pct)}`),badge(`MOS ${pct(v.margin_of_safety_pct)}`)].join('');
